...
from wtforms import (StringField, BooleanField, SubmitField,
                     IntegerField, PasswordField, SelectField, RadioField)
...
class ChangePassForm(Form):
    ...
    new_user_pass = PasswordField('NEW User Password:',
                          validators=[DataRequired(),
                          EqualTo('cnew_user_pass', message='Passwords must match')],
...
class ConfigThrowForm(Form):
    ...
    config_mode = SelectField('Configuration Mode:',
                          choices=[('user', 'User (non-enabled commands)'), ('conf', 'Config (conf t commands)')],
